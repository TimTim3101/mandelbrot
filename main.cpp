...
void scroll_callback(GLFWwindow *window, double xoffset, double yoffset) {
    double zoom = (yoffset > 0) ? .99f : 1.01f;

    double beforeX = centerX + (mouseX / (double)WIN_WIDTH - 0.5f) * scale * aspect;
    double beforeY = centerY + (mouseY / (double)WIN_HEIGHT - 0.5f) * scale * aspect;

    scale *= zoom;

    double afterX = centerX + (mouseX / (double)WIN_WIDTH - 0.5f) * scale * aspect;
    double afterY = centerY + (mouseY / (double)WIN_HEIGHT - 0.5f) * scale * aspect;
